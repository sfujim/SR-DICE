...
class AtariBuffer(object):
	...
	def add_start(self, state):
		...
		self.start_ptr = (self.start_ptr + 1) % self.max_start
		self.start_size = min(self.start_size + 1, self.max_start)


	def start_sample(self, batch_size):
		if self.start_size < batch_size:
	...
	def save(self, save_folder, chunk=int(1e5)):
		...
		np.save(f"{save_folder}_replay_info.npy", [self.ptr, chunk, self.start_ptr, self.start_size])

		np.save(f"{save_folder}_start_state.npy", self.start_state[:self.start_size])
	...
	def load(self, save_folder, size=-1):
		...
		self.ptr, chunk, self.start_ptr, self.start_size = np.load(f"{save_folder}_replay_info.npy")

		self.start_state[:self.start_size] = np.load(f"{save_folder}_start_state.npy")
