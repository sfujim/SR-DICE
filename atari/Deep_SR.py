...
class Deep_SR(object):
	...
	def train_OPE(self, replay_buffer, policy, batch_size=32):
		...
		w = (self.encoder_decoder.latent(state, action) * self.w).mean(1,keepdim=True)
		w_loss = F.mse_loss(w, reward)
	...
	def eval_policy(self, replay_buffer, policy, batch_size=100):
		...
		for _ in range(100):
			...
			sum_val += (1. - self.discount) * float((self.w * self.Q(start_state)[range(start_state.shape[0]), start_action.reshape(-1)].mean()).mean())
