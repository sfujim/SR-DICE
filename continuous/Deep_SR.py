...
class Deep_SR(object):
	...
	def train_OPE(self, replay_buffer, policy, batch_size=2048):
		...
		R = (self.encoder_decoder.latent(state, action) * self.w).mean(1,keepdim=True)
		w_loss = F.mse_loss(R, reward)
	...
	def eval_policy(self, replay_buffer, policy, batch_size=10000):
		...
		R = (1. - self.discount) * (self.w * self.critic(start_state, start_action)).mean()
		return float(R)